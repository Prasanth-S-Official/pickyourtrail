...
describe("LoanRequests Component", () => {
  ...
  test('should_handle_advanced_filter', async () => {
    ...
    jest.spyOn(axios, 'get').mockResolvedValue({ data: mockLoanRequestsData, status: 200 });
    ...
    const label=screen.getByLabelText(/Loan Type:/i)
    // Fill in the advanced filter inputs
    fireEvent.change(label, { target: { value: 'Loan 1' } });
    fireEvent.change(screen.getByLabelText('Model Start Date:'), { target: { value: '2007-10-01' } });
    fireEvent.change(screen.getByLabelText('Model End Date:'), { target: { value: '2023-10-07' } });
    fireEvent.change(screen.getByLabelText('Purchase Price:'), { target: { value: '600' } });
    fireEvent.change(screen.getByLabelText('Filter by Status:'), { target: { value: '1' } });

    // Click the Apply button
    fireEvent.click(screen.getByText('Apply'));

    // Check if the filtered data is displayed in the table
